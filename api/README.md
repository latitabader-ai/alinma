...
# Tawarruq engine — محرك التورق المنظم

طبقة تكوين العقد بين محرك الذكاء الاصطناعي والصرف.

- `pricing.py` — توجيه العقد (مرابحة/تورق) + تسعير سايبور بربح ثابت
- `market.py` — سوق السلع (وسيطان مستقلان) + حارس بيع العينة
- `engine.py` — آلة الحالات + البوابات الشرعية + سجل تدقيق متسلسل الهاش
- `api.py` — نقاط النهاية

الترتيب هو جوهر الحكم: شراء ← قبض ← بيع مؤجّل ← تسييل لطرف ثالث ← صرف.
الانتقالات غير المشروعة تُرفض (409 ترتيب · 422 عِينة · 428 بوابة مفتوحة).
